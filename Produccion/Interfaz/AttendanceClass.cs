...
>>>>>>> bf1dba918529b9e8fe8f30955f8e6ec0e9182058
        ...
        public int sta_ConnectTCP(ListBox lblOutputInfo, string ip, string port, string commKey, string name)
        {
            if (ip == "" || port == "" || commKey == "")
            {
                lblOutputInfo.Items.Add("*IP, Puerto o CmKey no pueden estar vacios !");
                return -1;// ip or port is null
            }

            if (Convert.ToInt32(port) <= 0 || Convert.ToInt32(port) > 65535)
            {
                lblOutputInfo.Items.Add("*Puerto ilegal!");
                return -1;
            }

            if (Convert.ToInt32(commKey) < 0 || Convert.ToInt32(commKey) > 999999)
            {
                lblOutputInfo.Items.Add("*CommKey ilegal!");
                return -1;
            }
            ...
            if (bIsConnected == true)
            {
                ...
                lblOutputInfo.Items.Add("Desconectado del dispositivo " + name+" - "+ DateTime.Now.ToString("dd-MM-yyyy hh:mm:ss tt"));
                //connected = false;
                return -2; //disconnect
            }
            ...
            if (axCZKEM1.Connect_Net(ip, Convert.ToInt32(port)) == true)
            {
                ...
                lblOutputInfo.Items.Add("Conectado con el dispositivo " + name + " - " + DateTime.Now.ToString("dd-MM-yyyy hh:mm:ss tt"));
            ...
            else
            {
                ...
                lblOutputInfo.Items.Add("*No se ha podido conectar con el dispositivo,ErrorCode=" + idwErrorCode.ToString());
                return idwErrorCode;
            }
        }
        ...
        public delegate DataGridView GetRealEventDtgHandler();
        private GetRealEventDtgHandler gRealEventDtgHandler;
        private DataGridView gRealEventDtg;

        public void sta_SetRTLogDtg(GetRealEventDtgHandler gvHandler)
        {
            gRealEventDtgHandler = gvHandler;
            gRealEventDtg = gRealEventDtgHandler();
        }
...
<<<<<<< HEAD
            ...
            axCZKEM1.OnEnrollFinger += new zkemkeeper._IZKEMEvents_OnEnrollFingerEventHandler(axCZKEM1_OnEnrollFinger);
=======
...
>>>>>>> bf1dba918529b9e8fe8f30955f8e6ec0e9182058
        ...
        //Method used to insert checkin/out
        void axCZKEM1_OnAttTransactionEx(string EnrollNumber, int IsInValid, int AttState, int VerifyMethod, int Year, int Month, int Day, int Hour, int Minute, int Second, int WorkCode)
        {
            ...
            //{
                ...
                //With this line you add real time events to a listbox, I delete it and add a datagridview
                ...
                gRealEventDtg.Rows.Add(EnrollNumber, GetUserName(EnrollNumber), time);
                //P.UserId = Convert.ToInt32(EnrollNumber);
                //P.Date = Convert.ToDateTime(time);
                //P.VerifyMode = VerifyMethod;
                //P.InsertAtteandance();
                throw new NotImplementedException();
            //}
            //catch(Exception ex)
        ...
        public string GetUserName(string UserId)
        {
            ...
            string strPassword = "";
            bool bEnabled = false;
            if (axCZKEM1.SSR_GetUserInfo(iMachineNumber, UserId, out strName, out strPassword, out iPrivilege, out bEnabled))//upload the user's information(card number included)
            {
        ...
        */
        ...
        public int sta_readAttLog(ListBox lblOutputInfo/*, DataTable dt_log*/)
        {
            ...
            if (axCZKEM1.ReadGeneralLogData(GetMachineNumber()))
            {
                ...
                {
                    P.UserId =Convert.ToInt32(sdwEnrollNumber);
